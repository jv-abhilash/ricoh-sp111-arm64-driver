...
__log_fn ="/tmp/ricoh_debug.log"#debug logging enabled
...
if __out_fn=="":
    ...
    __out = open(__out_fn,"wb") #generate to a real file
...
def term(fs):
    ...
    if lret!=0:
      log("COMMAND ERROR (ignored, checking output)")
...
os.makedirs(__uid, mode=0o775, exist_ok=True) #create temp dir with group write
__temp_dir = __uid +"/"
...
def out(fs): 
    log(fs)
    __out.write(fs.encode("ascii", errors="replace") if isinstance(fs, str) else fs)
    
#send string to output file
def outLine(fs):
    log(fs)
    __out.write((fs+'\r\n').encode("ascii", errors="replace") if isinstance(fs+'\r\n', str) else fs+'\r\n') # \r\n is essential here!
...
################################
doJobTrivial() #use trivial mode - ps2write segfaults in CUPS filter on ARM64
#doJobSimple()
#doJob()
log("printing: OK")
